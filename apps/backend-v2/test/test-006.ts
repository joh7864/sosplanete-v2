...
async function runTest006() {
  ...
  const logAnomaly = (testId: string, desc: string) => {
    anomalies.push(`| ${testId} | ${new Date().toLocaleDateString()} | Recette Auto | ${desc} | Haute | 🔴 À corriger |`);
  ...
  try {
    ...
    if (year2Response.status !== 200) {
        logAnomaly('TST-030', `Erreur création 2025-2026: ${year2Response.status}`);
    } else {
        results.push('TST-030: OK');
    }

    const iy1 = await prisma.instanceYear.findUnique({ where: { instanceId_schoolYear: { instanceId: instance.id, schoolYear: '2024-2025' } } });
    const iy2 = await prisma.instanceYear.findUnique({ where: { instanceId_schoolYear: { instanceId: instance.id, schoolYear: '2025-2026' } } });
    ...
    if (deletedIy1 || deletedIy2) {
        logAnomaly('TST-033', `Suppression d'instance n'a pas supprimé les InstanceYear`);
    } else {
        results.push('TST-033: OK');
    }

    console.log('DONE.', anomalies.length === 0 ? '0 anomalies' : `${anomalies.length} anomalies`);
    if (anomalies.length > 0) {
        const anomalyFile = path.join(__dirname, '..', '..', '..', '.docs', '4-reports', 'anomalies_recette.md');
        let content = fs.existsSync(anomalyFile) ? fs.readFileSync(anomalyFile, 'utf-8') : '';
        content += '\n' + anomalies.join('\n') + '\n';

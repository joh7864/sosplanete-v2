...
async function runRecette() {
  ...
  baseline.steps.push({ id: 'TST-002', status: 'SUCCESS', data: { email: admin.email, role: admin.role } });

  const anomalies: string[] = [];

  const logAnomaly = (testId: string, description: string, severity: string = 'Moyenne') => {
    anomalies.push(`| ${testId} | ${new Date().toLocaleDateString()} | Recette Auto | ${description} | ${severity} | 🔴 À corriger |`);
  ...
  try {
      ...
      if (response.status === 401) {
          baseline.steps.push({ id: 'TST-005', status: 'SUCCESS', data: { blocked: true } });
      } else {
          logAnomaly('TST-005', `L'accès à /instances sans token a renvoyé un statut ${response.status} au lieu de 401.`, 'Haute');
      }
  } catch (e) {
      logAnomaly('TST-005', `Erreur lors du test de protection des routes: ${e.message}`, 'Moyenne');
  }
  ...
  try {
    ...
    try {
        const result = await authService.validateUser(admin.email, 'wrong-password');
        if (result) {
            logAnomaly('TST-003', `Le login avec un mauvais mot de passe n'a pas été rejeté (validateUser a renvoyé un utilisateur).`, 'Moyenne');
        } else {
            baseline.steps.push({ id: 'TST-003', status: 'SUCCESS', data: { blocked: true } });
        }
    } catch (e) {
        baseline.steps.push({ id: 'TST-003', status: 'SUCCESS', data: { blocked: true, message: e.message } });
    }
    ...
    try {
        ...
        if (users.length > 0) {
            baseline.steps.push({ id: 'TST-006', status: 'SUCCESS', data: { count: users.length } });
        } else {
    ...
    try {
        ...
        baseline.steps.push({ id: 'TST-007', status: 'SUCCESS', data: { id: createdUser.id, email: createdUser.email } });
    } catch (e) {
        logAnomaly('TST-007', `Erreur création utilisateur: ${e.message}`, 'Haute');
    }
    ...
    try {
        ...
        logAnomaly('TST-008', `Le doublon d'email n'a pas été bloqué.`, 'Moyenne');
    } catch (e) {
        if (e.status === 409 || e.message.includes('Cet email est déjà utilisé')) {
            baseline.steps.push({ id: 'TST-008', status: 'SUCCESS', data: { blocked: true } });
        } else {
            logAnomaly('TST-008', `Erreur inattendue pour doublon email: ${e.message}`, 'Basse');
        }
    }
    ...
    try {
        if (createdUser) {
            ...
            baseline.steps.push({ id: 'TST-009', status: 'SUCCESS', data: { deleted: true } });
        }
    } catch (e) {
        logAnomaly('TST-009', `Erreur suppression utilisateur: ${e.message}`, 'Haute');
    }

    // TST-015: Import Référentiel Actions (DATA-01)
    console.log('--- TST-015: Import Référentiel Actions ---');
    try {
        const refFile = path.join(fctPath, 'SOSPlanete_referentiel_actions_V3.csv');
        const refBuffer = fs.readFileSync(refFile);
        const refResult = await (actionRefService as any).importFromCSV(refBuffer);
        baseline.steps.push({ id: 'TST-015', status: 'SUCCESS', data: refResult });
    } catch (e) {
    ...
    try {
        ...
        const refBuffer = fs.readFileSync(refFile);
        const refResult = await (actionRefService as any).importFromCSV(refBuffer);
        baseline.steps.push({ id: 'TST-016', status: 'SUCCESS', data: refResult });
    } catch (e) {
    ...
    try {
        ...
        baseline.steps.push({ id: 'TST-014', status: 'SUCCESS', data: { count: catalog.length } });
    } catch (e) {
        logAnomaly('TST-014', `Erreur catalogue actions: ${e.message}`, 'Moyenne');
    }
    ...
    try {
        const catCsv = fs.readFileSync(path.join(docsPath, 'Catégories.csv'), 'utf-8');
        const catResult = await categoryService.importCsv(instance.id, catCsv, schoolYear, user);
        baseline.steps.push({ id: 'TST-024', status: 'SUCCESS', data: catResult });
    } catch (e) {
        logAnomaly('TST-024', `Erreur import Catégories.csv: ${e.message}`, 'Haute');
    }

    // TST-026: Import Équipes
    console.log('--- TST-026: Import Équipes ---');
    try {
        const teamCsv = fs.readFileSync(path.join(docsPath, 'Neyron_equipes.csv'), 'utf-8');
        const teamResult = await teamService.importCsv(instance.id, teamCsv, schoolYear, user);
        baseline.steps.push({ id: 'TST-026', status: 'SUCCESS', data: teamResult });
    } catch (e) {
        logAnomaly('TST-026', `Erreur import Neyron_equipes.csv: ${e.message}`, 'Haute');
    }

    // TST-027: Import Actions Réalisées
    console.log('--- TST-027: Import Actions Réalisées ---');
    try {
        const actionsDoneCsv = fs.readFileSync(path.join(docsPath, 'Neyron_actions_realisees.csv'), 'utf-8');
        const actionsResult = await trackingService.importActionsCsv(instance.id, actionsDoneCsv, schoolYear);
        baseline.steps.push({ id: 'TST-027', status: 'SUCCESS', data: actionsResult });
    } catch (e) {
        logAnomaly('TST-027', `Erreur import Neyron_actions_realisees.csv: ${e.message}`, 'Haute');
    }

    // TST-028: Calcul Impact
    console.log('--- TST-028: Calcul Impact ---');
    try {
        const impact = await impactService.calculateImpact(schoolYear, instance.id);
    ...
    console.log('--- TST-021, 022, 023, 013: Gestion manuelle et suppression ---');
    try {
        ...
        baseline.steps.push({ id: 'TST-021', status: 'SUCCESS', data: { teamId: team.id } });

        // TST-022: Créer groupe
        const group = await teamService.createGroup(team.id, 'Groupe A', '#00FF00');
        baseline.steps.push({ id: 'TST-022', status: 'SUCCESS', data: { groupId: group.id } });

        // TST-023: Créer enfant
        const child = await teamService.createChild(group.id, 'Alice');
        baseline.steps.push({ id: 'TST-023', status: 'SUCCESS', data: { childId: child.id } });
    ...
    if (anomalies.length > 0) {
        try {
            const anomalyFile = path.join(__dirname, '..', '..', '..', '.docs', '4-reports', 'anomalies_recette.md');
            const currentContent = fs.readFileSync(anomalyFile, 'utf-8');
            fs.writeFileSync(anomalyFile, currentContent + '\n' + anomalies.join('\n') + '\n');
            console.log(`Rapport d'anomalies mis à jour avec ${anomalies.length} entrée(s).`);
        } catch (fileErr) {
            console.error('Erreur lors de l\'écriture du rapport d\'anomalies:', fileErr);
        }
    }
    await app.close();
  }

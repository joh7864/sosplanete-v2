...
async function main() {
  ...
  for (const action of localActions) {
    const physicalCat =
      action.category?.name || action.actionRef?.category || 'Général';
    ...
    if (
      !descSF ||
      descSF.includes("effectuez l'action écologique correspondante")
    ) {
      let introFun =
    ...
    const hp =
      calculated > 0 ? calculated : action.evoeMission?.pointsGagnes || 10;
  ...
  const outputPathDocs = path.resolve(
    __dirname,
    '../../../../.docs/MISSIONS_FUTURISTES_EVOE.csv',
  );
  const outputPathRoot = path.resolve(
    __dirname,
    '../../../../MISSIONS_FUTURISTES_EVOE.csv',
  );

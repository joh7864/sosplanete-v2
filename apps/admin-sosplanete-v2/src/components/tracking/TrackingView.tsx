...
export function TrackingView({ 
  ...
  }, [instanceId, schoolYear, instanceYearId]);
  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-3 py-3 px-5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'indicators' ? 'bg-emerald-50 text-emerald-600 shadow-sm border border-emerald-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
          >
            <TrendingUp size={16} /> Impact
          </button>
          <button
            onClick={() => setActiveTab('actions')}
            className={`flex items-center gap-3 py-3 px-5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'actions' ? 'bg-emerald-50 text-emerald-600 shadow-sm border border-emerald-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
          >
            <BarChart2 size={16} /> Suivi actions
          </button>
          <button
            onClick={() => setActiveTab('animals')}
            className={`flex items-center gap-3 py-3 px-5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'animals' ? 'bg-emerald-50 text-emerald-600 shadow-sm border border-emerald-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
          >
      ...
      <TrackingMatrix 
        ...
        visiblePeriodIndices={visiblePeriodIndices}
        computedWeeklyTotals={computedWeeklyTotals}
        computedTableGrandTotal={computedTableGrandTotal}
        teams={teams}
        groups={groups}
        ...
        setHideEmptyPeriods={setHideEmptyPeriods}
        isFullscreen={isFullscreen}
        setIsFullscreen={setIsFullscreen}
      />

      {/* Global Evolution Chart */}
      <TrackingCharts chartData={chartData} />

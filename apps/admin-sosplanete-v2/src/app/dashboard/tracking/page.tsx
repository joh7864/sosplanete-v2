          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'graphic' && (
              <button 
                ...
                disabled={isRecalculating}
                className={`p-2.5 rounded-xl bg-white text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all border border-slate-100 shadow-sm flex items-center justify-center group ${isRecalculating ? 'opacity-50 cursor-not-allowed' : ''}`}
                title="Recalculer le classement inter-écoles"
              >
                <RefreshCcw size={20} className={`${isRecalculating ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'}`} />
              </button>
            )}
    ...
    if (isPlaying && history.length > 0 && selectedPeriod !== null) {
      interval = setInterval(() => {
        setSelectedPeriod(prev => {
          if (prev === null) return history[0].period;
  ...
  const chartData = useMemo(() => {
    return [...rankings]
      ...
      .map(r => ({
        ...
        isCurrent: r.instanceId === instanceId,
        color: colorMap[r.instanceId] || '#64748b',
      }));
  }, [rankings, instanceId, colorMap]);
  ...
  if (history.length === 0) return (
    <div className="p-8">
      <GlassCard className="p-10 rounded-3xl flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-slate-500">Cliquez sur le bouton ↺ pour générer l'historique.</p>
      </GlassCard>
    </div>
  );
  ...
  return (
    <div className="p-4 space-y-4 flex flex-col w-full">
      ...
      <GlassCard className="w-full rounded-3xl overflow-hidden relative" style={{ padding: '28px 32px 0px' }}>
        ...
        {/* Filigrane date en haut à droite - supprimé ici, réintégré en bas à droite */}
        ...
        {/* AXE X + CONTRÔLES EN DESSOUS DES BARRES — layout comme la référence Flourish */}

      ...
      >
        ...
        <ZoomIn>
          <div className="game-text-result">
            ...
            {gameAborted && (
              <>
                <strong>Tu as abandonné ! </strong>Ce n'est pas grave, tu feras
                mieux la prochaine fois !<br></br>
              </>
            )}
        ...
        >
          {resultats.map((item, index) => (
            <div
              className="game-res-item"
              key={index}
              style={{
                backgroundColor: item.statut
                  ? "rgb(255, 243, 234, .3)"
                  : "rgb(255, 190, 0, .3)",
              }}
            >
              <div className="game-res-item-icon">
                {item.statut ? <img src={etoile} /> : <img src={redcross} />}
              </div>
              <div className="game-res-item-texte">
                {!item.statut ? item?.reponse : ""}
              </div>
              <div
                className="game-res-item-texte"
                style={item.statut ? styleTextGain : styleTextPerte}
              >
